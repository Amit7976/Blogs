...
interface Blog {
    ...
    status: string;
    created_at: Date;
    updated_at?: Date; // optional if not always present
}
        ...
        <div className="bg-white hover:bg-gray-50 dark:bg-neutral-900 dark:hover:bg-neutral-800 w-full col-span-1 h-fit p-2 rounded-lg shadow-lg hover:shadow-2xl hover:shadow-red-100 hover:scale-105 will-change-transform duration-300">
            <Link href={`/blogs/${blog.category}/${blog._id}`} target="_blank">
                <Image
                    width={300}
                    height={300}
                    className="shrink-0 w-full aspect-video rounded-lg"
            ...
            <div className="pt-2">
                <Link href={`/blogs/${blog.category}/${blog._id}`} target="_blank">
                    <span className="text-lg text-gray-600 dark:text-neutral-600 line-clamp-1 font-semibold">
                        {blog.title}
                    </span>
                </Link>
            </div>
            <div className="flex justify-between items-center pt-3">
                ...
                <p
                    className={`py-1.5 px-5 inline-flex items-center gap-x-1 text-xs font-medium ${blog.status === "publish"
                            ? "text-teal-800 rounded-lg dark:text-teal-500"
                            : "text-blue-800 rounded-lg dark:text-blue-500"
                        }`}
                >
            ...
            <div className="pt-3">
                <div className="flex items-center justify-between gap-1">
                    ...
                    <MdOutlineDeleteOutline
                        className="bg-gray-50 hover:bg-red-100 hover:text-gray-600 duration-300 active:scale-90 rounded w-full h-10 p-2.5 cursor-pointer"
